...
const SkillsShowcase = () => {
  ...
  const skillCategories = [
    ...
    },
    {
      ...
      skills: [
        ...
        { name: "Responsive Design", level: 90, icon: "Smartphone", color: "from-teal-500 to-cyan-500" }
      ]
    },
    {
      title: "Cybersecurity",
      icon: "Shield",
      skills: [
        { name: "Web Security", level: 60, icon: "Lock", color: "from-red-500 to-pink-500" },
        { name: "Penetration Testing", level: 45, icon: "Search", color: "from-orange-500 to-red-600" },
        { name: "Network Security", level: 55, icon: "Wifi", color: "from-purple-500 to-indigo-500" },
        { name: "Vulnerability Assessment", level: 50, icon: "AlertTriangle", color: "from-yellow-500 to-orange-600" }
      ]
    }
  ];
  ...
  const SkillBar = ({ skill, index, categoryIndex }) => {
    ...
    useEffect(() => {
      if (isInView && !animatedSkills.has(skillKey)) {
    ...
    }, [isInView, skill.level, skillKey, index, categoryIndex, animatedSkills]);
        ...
        <div className="flex items-start space-x-4">
          ...
          <div className="flex-1">
            ...
            <p className="font-body text-text-secondary leading-relaxed">
              I'm constantly expanding my skill set through hands-on projects, online courses, and 
              real-world applications. My focus is on building secure, scalable, and user-friendly 
              web applications while staying current with industry best practices.
            </p>
          </div>
        </div>
      </motion.div>
